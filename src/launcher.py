...
class GroupWidget(QWidget):
    ...
    def __init__(
        ...
        self.icon_button.setGeometry(
            QRect(scaled(20), 0, self.icon_button.width(), self.icon_button.height())
        )
        ...
        self.title_label.setGeometry(
            QRect(
                0, scaled(85 + 11), self.width(), self.title_label.height() + scaled(10)
            )
        )
        ...
        if shortcut is not None:
            ...
            self.hotkey_label.setGeometry(
                QRect(
                    0,
                    scaled(98 + 17 + 17) + self.title_label.sizeHint().height(),
                    self.width(),
    ...
    def get_widget_position(index: int) -> QPoint:
        ...
        y = ceil(index / 4) - 1
        position = QPoint(
            GroupWidget.size().width() * x,
            (GroupWidget.size().height() + scaled(40)) * y,
        )
...
class LowerWidget(QMainWindow):
    ...
    def __init__(self, add_ons: dict[str, ModuleType]) -> None:
        ...
        if check_setting("lower_position"):
            self.lower_position = QPoint(
                get_setting("lower_position")[0], get_setting("lower_position")[1]
            )
        else:
            ...
            self.lower_position = QPoint(
                screen.width() // 2 - self.size().width() // 2,
                screen.height() - 60 - self.size().height(),
            )

        self.icon: QPixmap = QPixmap(get_icon("icon.png")).scaled(
            scaled(35),
            scaled(35),
        ...
        self.setHidden(get_setting("lower-hidden")) if check_setting(
            "lower-hidden"
        ) else self.show()
        self.main_window.setHidden(get_setting("upper-hidden")) if check_setting(
            "upper-hidden"
        ) else self.show()
    ...
    def toggle_windows(self) -> None:
        if self.isHidden():
            ...
            self.active_windows = (
                x for x in QApplication.allWindows() if x.isVisible()
            )
            for window in self.active_windows:
                window.hide()
    ...
    def mouseReleaseEvent(self, a0: QMouseEvent) -> None:
        if self._moved:
            ...
            apply_setting(
                "lower_position", [self.lower_position.x(), self.lower_position.y()]
            )
        else:
...
class MainWindow(QWidget):
    def __init__(
        self, add_ons: dict[str, ModuleType], parent: QWidget | None = None
    ) -> None:
        super().__init__(parent)
        ...
        if check_setting("upper_position"):
            self.upper_position = QPoint(
                get_setting("upper_position")[0], get_setting("upper_position")[1]
            )
        else:
            desktop = QApplication.desktop()
            primary_screen_index = desktop.primaryScreen()
    ...
    def get_window_size(self) -> QSize:
        ...
        window_size = QSize(
            GroupWidget.size().width() * x,
            (GroupWidget.size().height() + scaled(40)) * y - scaled(40),
        )
        return window_size + QSize(
            scaled(20 + 20), scaled(40 + 40)
        )  # adding left, right, top and bottom padding
    ...
    def add_widget(self, index: int, add_on_name: str) -> None:
        ...
        widget = GroupWidget(
            self, index, title, icon_path, hover_icon_path, shortcut, activate
        )
    ...
    def mouseReleaseEvent(self, a0: QMouseEvent) -> None:
        if self._moved:
            ...
            apply_setting(
                "upper_position", [self.upper_position.x(), self.upper_position.y()]
            )
        self._moved = False
        self._offset = None
        return super().mouseReleaseEvent(a0)
