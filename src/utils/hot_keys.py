...
class HotKeys:
    ...
    def add_global_shortcut(shortcut: str, callback: Callable) -> None:
        ...
        hot_keys = {
            _shortcut: lambda _shortcut=_shortcut: HotKeys._call_callbacks(_shortcut)
            for _shortcut in HotKeys._shortcuts_and_callbacks.keys()
        }
    ...
    def format_shortcut_string(key: str) -> str:
        ...
        return "+".join(
            f"<{k.lower().strip()}>" if k.lower().strip() in x else k.lower().strip()
            for k in key.split("+")
        )
