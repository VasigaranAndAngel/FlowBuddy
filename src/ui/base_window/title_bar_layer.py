...
class TabButton(QWidget):
    ...
    def get_tab_button_position(index: int) -> QPoint:
        return QPoint(
            scaled(20) + (TabButton.size().width() + scaled(16)) * scaled(index),
            scaled(6),
        )
    ...
    def paintEvent(self, a0: QPaintEvent) -> None:
        ...
        painter.drawRoundedRect(
            0, 0, self.size().width(), self.size().height(), scaled(12), scaled(12)
        )
        painter.setPen(QPen(self.palette().text().color()))
        painter.drawText(
            self.rect().adjusted(scaled(20), scaled(5), -scaled(52), 0),
    ...
    def mouseReleaseEvent(self, a0: QMouseEvent) -> None:
        ...
        self.tab_moved.emit(
            self.tab_id
        )  # for update the positions of dragging TabButton
...
class TitleBarLayer(QWidget):
    def __init__(
        self,
        title_bar: Optional[Literal["title", "tab", "hidden"]] = None,
    ...
    def _init_for_tabs(self) -> None:
        ...
        self.add_button.move(
            scaled(20),
            scaled(50 if self.mode == "tab" else 34) // 2
            - self.add_button.height() // 2,
        )
    ...
    def _tab_moving(self, tab_id: int):
        ...
        changed = (
            self.tabs[tab_id].x() - TabButton.get_tab_button_position(current_order).x()
        )
    ...
    def _reset_tab_positions(self) -> None:
        ...
        for index, tab_id in enumerate(self.tabs_order):
            ...
            if (
                tab._offset is None and tab.pos() != pos
            ):  # check if the tab is being dragged.
                tab.move(pos)
    ...
    def _set_button_position(self) -> None:
        ...
        self.buttons.move(
            self.width() - self.buttons.width() - scaled(20),
    ...
    def _set_add_button_position(self) -> None:
        if not self.add_button.isHidden():
            ...
            self.add_button.move(
                TabButton.get_tab_button_position(len(self.tabs)).x(),
                self.add_button.y(),
            )
    ...
    def mouseMoveEvent(self, a0: QMouseEvent) -> None:
        if self._offset_for_drag is None:
            return
        content_margins = self._parent.layout().contentsMargins()
        self._parent.move(
            a0.globalPos()
            - self._offset_for_drag
