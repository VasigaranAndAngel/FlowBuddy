...
def add_base_window(
    widget: QWidget | TabWidget,
    title_bar: Literal["title", "tab", "hidden"],
    parent: QWidget | None = None,
) -> None:
    if title_bar not in ["title", "tab", "hidden"]:
        raise ValueError(
            f"Invalid title_bar option: '{title_bar}'. title_bar should be 'title' or 'tab'"
        )
    ...
    shadow_layer_layout.addWidget(
        title_bar_layer := TitleBarLayer(title_bar, shadow_layer)
    )
...
class BaseWindow(QWidget, Buttons):
    def __init__(
        self, hide_title_bar: bool = False, parent: QWidget | None = None
    ) -> None:
        super().__init__()
...
class TabsWindow(TabWidget, Buttons):
    ...
    def addTab(
        self, widget: QWidget, label: str, icon: QIcon | None = None
    ) -> TabsWindow.TabIndex:
