...
from .nodes import GroupNode, SubNodeManager, TaskNode
...
class MainWindow(BaseWindow):
    ...
    def _update_edit_mode(self) -> None:
        ...
        self._add_new_group_layout.setContentsMargins(
            0, scaled(25) if self._edit_mode else 0, 0, 0
        )
        self.adjustSize()
    ...
    def _on_task_node_departed(self, task_node: TaskNode) -> None:
        print(
            f"{self._nodes[task_node.task_class.task_id]} is departed from {self._nodes[Data.get_group_id_of_task(task_node.task_class.task_id)]}."
        )
