...
class FileChooseButton(TextButton):
    class InnerButton(TextButton):
        ...
        def define_geometries(self):
            ...
            width = geo.width() // 2
            height = geo.height()
            self.exposed_geometry = QRect(
                width if self.side == "right" else 0, 0, width, height
            )
...
class GroupDialog(BaseDialog):
    ...
    def exec(self) -> Any:
        ...
        self.setGeometry(
            geo.adjusted(
                cursor_pos.x() - geo.width() // 2,
                cursor_pos.y() - geo.height() // 2,
                0,
                0,
            )
        )
...
class TaskDialog(BaseDialog):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        file_choose_button.file_button.clicked.connect(
            lambda: self._choose_file("file")
        )
        file_choose_button.folder_button.clicked.connect(
            lambda: self._choose_file("folder")
        )
    ...
    def _choose_file(self, type: Literal["file", "folder"]):
        ...
        if type == "file":
            ...
            self._file_path = QFileDialog.getExistingDirectory(
