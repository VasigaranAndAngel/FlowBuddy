...
class SelectedColorWidget(QWidget):
    def __init__(self, color: str = "#000000"):
        ...
        self.copy_svg_icon = QSvgWidget(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "icons", "copy-icon.svg"
            )
        )
        ...
        self.delete_widget_btn.setIconSize(self.delete_widget_btn.iconSize() * 0.5)
        ...
        self.color_display_box.setStyleSheet(f"background-color: {self.color};")
        ...
        self.layout.addItem(
            QSpacerItem(0, 0, QSizePolicy.Minimum, QSizePolicy.Expanding), 3, 0, 1, 10
        )
...
class ColorPickerWidget(ColorPicker):
    ...
    def exit_btn_clicked(self):
        self.hide()
        buddy_color_picker.add_selected_color_signal.emit(f"#{self.ui.hex.text()}")
...
class BuddyColorPicker(BaseWindow):
    ...
    def __init__(self):
        ...
        self.desktop_color_picker.clicked.connect(self.start_desktop_color_picker)
    ...
    def add_selected_color(self, color: str):
        if color not in self.added_colors:
            ...
            self.selected_color_widgets = len(self.findChildren(SelectedColorWidget))
            self.setMinimumHeight(300 + 100 * self.selected_color_widgets)
            if self.selected_color_widgets > 5:
                for ind, selected_color_widget in enumerate(
...
class MagnifierWidget(QWidget):
    ...
    def capture(self):
        if self.track_color:
            ...
            qImg = QImage(
                screen_np.data, width, height, bytesPerLine, QImage.Format_RGB888
            )
            ...
            for i in range(center % pixel_size, 340, pixel_size):
                painter.drawLine(i - pixel_size // 2, 0, i - pixel_size // 2, 340)
                painter.drawLine(0, i - pixel_size // 2, 340, i - pixel_size // 2)

            pen = QPen(QColor("white"), 1, Qt.SolidLine)
            painter.setPen(pen)
            painter.drawRect(
                center - pixel_size // 2 - 2,
                center - pixel_size // 2 - 2,
                pixel_size + 3,
                pixel_size + 3,
            )
...
AddOnBase().activate = (
    lambda: buddy_color_picker.show()
    if buddy_color_picker.isHidden()
    else buddy_color_picker.hide()
)
